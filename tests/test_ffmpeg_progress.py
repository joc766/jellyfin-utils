...
def test_ffmpeg_progress(testing_mode: bool = False, verbose: bool = False):
    test_file_path = DATA_DIR / "ffmpeg-output.txt"
    test_file = test_file_path.open(mode="r", buffering=1, encoding="utf-8")
    duration = 1280.672
    tracker = FFmpegProgressTracker()
    with FFmpegProgressRender(test_file_path.stem, duration) as render:
